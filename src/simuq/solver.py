...
def solve_aligned(
    ...
    if solver == "least_squares":
        ...
        t = time.time() - full_start_time
        #raise Exception('SUCCESS', t)
        return True
        ...
        # first hamiltonian
        ham, gtime = qs.evos[0]
        targets = { str(term): coefficient for term, coefficient in ham.ham }
        
        gsol = [None] * len(mach.lvars)
        gswitch = [[0 for _ in line.inss] for line in mach.lines]
        
        for line_i in range(len(mach.lines)):
            line = mach.lines[line_i]
            for ins_i in range(len(line.inss)):
                ins = line.inss[ins_i]
                ...
                for term, coefficient in ins.h.ham:
                    target = targets.pop(str(term), 0)
                    eqns.append((coefficient, target))
                    for var in coefficient.vars:
                        if var not in map:
                            map[var] = next
                            next += 1
                    if abs(target) > tol:
                ...
                if switch:
                    ...
                    x = opt.root(f, np.ones(len(map)), tol=tol)
                    if not x.success:
                        raise Exception("Failed to solve the equation.")
                    vals = x.x
                else:
                    vals = np.zeros(len(map))
                
                for i in map.keys():
                    if gsol[i.index] is not None:
                        raise Exception('uhhh')
                    gsol[i.index] = vals[map[i]]
        
        if not all(abs(v) < tol or k == '<productHamiltonian d={}>' for k, v in targets.items()):
            raise Exception('Not all terms are covered.')
